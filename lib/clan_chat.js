...
async function authorClan(userId) {
  ...
  const r = await query(
    `SELECT c.name, c.banner FROM clan_members cm JOIN clans c ON c.id = cm.clan_id WHERE cm.user_id = $1`, [userId]);
  if (!r.rows[0]) return null;
  const b = palette.resolveBanner(r.rows[0].banner);
  return { name: r.rows[0].name, primary: b.primaryHex, glyph: b.glyph };
}

// SQL fragment + mapper that add author_clan to message/post rows.
...
const AUTHOR_CLAN_COLS = 'ac.name AS author_clan_name, ac.banner AS author_clan_banner';
function withAuthorClan(row) {
  const { author_clan_name, author_clan_banner, ...rest } = row;
  if (!author_clan_name) return { ...rest, author_clan: null };
  const b = palette.resolveBanner(author_clan_banner);
  return { ...rest, author_clan: { name: author_clan_name, primary: b.primaryHex, glyph: b.glyph } };
}
