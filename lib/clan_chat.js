...
const AUTHOR_CLAN_COLS = 'ac.name AS author_clan_name, ac.banner AS author_clan_banner, asu.site_role AS author_site_role';
function withAuthorClan(row) {
  const { author_clan_name, author_clan_banner, author_site_role, ...rest } = row;
  ...
  return { ...rest, author_clan: { name: author_clan_name, primary: b.primaryHex, glyph: b.glyph }, author_staff };
}
