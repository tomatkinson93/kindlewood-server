...
// ── Registered games ──
// Briar is always a full 6-seat Court (so the Heron is always dealt); a solo
// host fills the other five seats with AI courtiers.
registerGame('briar', { name: 'Briarwood Court', minPlayers: 6, maxPlayers: 6 });
...
// The first four have Briar personalities; Bramblefoot and Quill play neutral,
// so one human can fill a full 6-seat table (e.g. a Heron game) with AI.
...
// Host fills every empty seat with a random, not-yet-seated courtier.
function fillAI(room, userId) {
  if (room.hostId !== userId) throw new Error('Only the host can add courtiers');
  if (room.status !== 'lobby') throw new Error('Game already started');
  const taken = new Set(room.players.filter(p => p.isAI).map(p => p.name));
  const pool = AI_ROSTER.filter(n => !taken.has(n));
  for (let i = pool.length - 1; i > 0; i--) {             // Fisher–Yates
    const j = crypto.randomInt(i + 1);
    [pool[i], pool[j]] = [pool[j], pool[i]];
  }
  let added = 0;
  while (room.players.length < room.maxPlayers && pool.length) {
    const pick = pool.pop();
    room.players.push({ id: 'ai:' + pick.replace(/\s+/g, '_'), name: pick, isAI: true });
    added++;
  }
  if (!added) throw new Error(room.players.length >= room.maxPlayers ? 'No empty seats' : 'No more courtiers available');
  touch(room);
  broadcast(room, { type: 'lobby_update', room: publicView(room) });
  return room;
}
...
// Which AI seats is the engine currently waiting on? Briar delegates to the
// engine's pendingSeats() (single source of truth); squirrel keeps its own
// per-phase logic for now. Several at once during a reaction window.
function pendingAiSeats(room) {
  const g = room.state, E = room.engine;
  const seatIsAI = s => { const o = room.seats.find(x => x.seat === s); return !!(o && o.isAI); };
  if (room.gameType === 'squirrel') {
    const P = g.pending;
    if (g.phase === 'turn') { const a = E.currentActor(g); return seatIsAI(a) ? [a] : []; }
    if (g.phase === 'squirrel' || g.phase === 'magpie' || g.phase === 'foxdare')
      return seatIsAI(P.actorSeat) ? [P.actorSeat] : [];
    if (g.phase === 'storm') return E.stormPending(g).filter(seatIsAI);
    return [];
  }
  return E.pendingSeats(g).filter(seatIsAI);
}
function pendingAiSeat(room) { const s = pendingAiSeats(room); return s.length ? s[0] : null; }

// Reaction windows: many seats answer the same question at once.
const REACTION_PHASES = new Set(['challengeAction', 'block', 'titheBlock', 'storm']);
...
// AFK human. AI moves are jitter-paced per seat (_driveAi) so they don't feel instant.
const AI_TICK_MS = 200;               // clock granularity (fine enough for scattered reaction timing)
const AI_MIN_DELAY = 700;             // per-AI-move pacing floor (ms) — turns, reveals, consults
const AI_MAX_DELAY = 1600;            // per-AI-move pacing ceiling (ms)
const AI_REACT_MIN = 250;             // reaction windows: each AI answers on its own
const AI_REACT_MAX = 1500;            //   random clock, so sign-offs land scattered, not in seat order
const SEASON_REVEAL_MS = 3300;        // Briar: clients play a season-draw reveal this long; the table holds for it
...
const BRIAR_PHASE_TIMEOUT = {
  ...
  titheBlock:      30000,   // → every seat still owing pays (no silent Heron claim)
};
...
function _refreshDeadline(room) {
  ...
  if (sig !== room.phaseSig) {
    ...
    // A human's clock starts after any season reveal, not underneath it.
    const now = Date.now();
    room.deadlineAt = Math.max(now, _seasonHoldUntil(room, now)) + ms;
  }
}
...
function _enforceTimeout(room) {
  ...
  switch (g.phase) {
    ...
    case 'loseInfluence':
      ...
      break;
    case 'titheBlock':
      for (const s of E.pendingSeats(g)) E.titheRespond(g, s, true);
      break;
    case 'consult': {
...
// Drive the AI seats the engine is waiting on. Each seat gets its own due time
// when a decision window opens (kept while the window is unchanged — _phaseSig
// ignores passes, so answers don't reshuffle the others). In a reaction window
// every AI answers independently on a short random clock, so the table signs
// off together in a scattered order; single-actor decisions keep a longer
// "thinking" pace. Every seat that is due this cycle acts, earliest first.
function _driveAi(room, aiSeats, now) {
  const g = room.state;
  const sig = _phaseSig(g, room.gameType);
  if (!room.aiDue || room.aiDue.sig !== sig) room.aiDue = { sig, at: {} };
  const react = REACTION_PHASES.has(g.phase);
  const [lo, hi] = react ? [AI_REACT_MIN, AI_REACT_MAX] : [AI_MIN_DELAY, AI_MAX_DELAY];
  const at = room.aiDue.at;
  const start = Math.max(now, _seasonHoldUntil(room, now));   // let a season reveal play out first
  for (const s of aiSeats) if (at[s] == null) at[s] = start + lo + Math.random() * (hi - lo);
  const due = aiSeats.filter(s => now >= at[s]).sort((a, b) => at[a] - at[b]);
  for (const s of due) {
    if (g.phase === 'gameover' || room.status !== 'playing') break;
    if (_phaseSig(g, room.gameType) !== sig) break;      // the window moved on (e.g. someone challenged)
    if (!pendingAiSeats(room).includes(s)) continue;
    const decision = room.engine.aiResolve(g, s);
    if (decision) gameActionBySeat(room, s, decision);
  }
}

// Briar: a newly drawn season (seasonSeq changed) plays a reveal on every
// client, so the table holds still for SEASON_REVEAL_MS from when the server
// first sees it. Returns the hold's end time (0 when there is none).
function _seasonHoldUntil(room, now) {
  const g = room.state;
  if (room.gameType !== 'briar' || !g || g.seasonSeq == null) return 0;
  if (room.seasonSeqSeen !== g.seasonSeq) {
    room.seasonSeqSeen = g.seasonSeq;
    room.seasonHoldUntil = now + SEASON_REVEAL_MS;
  }
  return room.seasonHoldUntil || 0;
}

// One clock cycle: for each live room, either advance the due AI decisions or,
// if the room is waiting on a human past the deadline, apply the default.
// `nowOverride` lets tests fast-forward (pass Infinity: every AI is due).
function _serverTick(nowOverride) {
  ...
  for (const room of rooms.values()) {
    ...
    try {
      ...
      const aiSeats = pendingAiSeats(room);
      if (aiSeats.length) {
        _driveAi(room, aiSeats, now);
...
function rematch(room, userId) {
  ...
  room.phaseSig = null;
  room.aiDue = null;
  room.seasonSeqSeen = null;
  room.seasonHoldUntil = 0;
  room.absent = null;
...
function markAbsent(room, userId) {
  ...
  // Already cast out? There's nothing to pause for — they've left a game
  // they can no longer play. Treat it as leaving (the table plays on, or ends
  // and records their loss if no humans remain).
  const gp = room.state && room.state.players.find(p => p.seat === seat.seat);
  if (gp && !gp.alive) { forfeit(room, userId); return; }
...
function _endIfNoHumans(room) {
  ...
  room.deadlineAt = null; room.phaseSig = null;
  touch(room);
  _recordAbandoned(room);
  return true;
}

// Every human walked away before the game finished: it doesn't matter which AI
// would have won — each human who sat at the table takes a loss.
function _recordAbandoned(room) {
  try {
    const stats = require('./game_stats_store');
    const humans = (room.seats || []).filter(s => !(typeof s.id === 'string' && s.id.startsWith('ai')));
    const solo = humans.length <= 1;
    for (const s of humans) stats.record(s.id, room.gameType, false, { solo }).catch(() => {});
...
module.exports = {
  ...
  pendingAiSeat, pendingAiSeats, _serverTick, _enforceTimeout, _refreshDeadline, _convertSeatToAI,
  addAI, fillAI, removeAI, chat, typing, cursor, subscribe, unsubscribe, broadcast, publicView, stats, summary,
