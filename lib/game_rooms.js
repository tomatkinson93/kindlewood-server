...
function _convertSeatToAI(room, userId, entry) {
  ...
  broadcast(room, { type: 'seat_converted', seat: seat.seat, name: seat.name,
    message: `${seat.name} disconnected — an AI will play their hand.` });
  touch(room);
...
// button). Unlike a disconnect this is deliberate: forfeit immediately — hand
// their seat to the AI right now, no grace — and the table plays straight on.
function forfeit(room, userId) {
  ...
  if (room.hostId === userId) _migrateHost(room, userId);
  _convertSeatToAI(room, userId, entry);
}
