...
function createRoom({ gameType, hostId, hostName, visibility, maxPlayers, difficulty, seasons }) {
  ...
  const room = {
    ...
    // Court Seasons variant (Briar only). Strict boolean whitelist.
    seasons: gameType === 'briar' && (seasons === true || seasons === 'true'),
    maxPlayers: max,
...
function start(room, userId) {
  ...
  if (engine) {
    room.engine = engine;
    room.state = engine.create(room.seats, { difficulty: room.difficulty, rng, seed, seasons: !!room.seasons });
  }
