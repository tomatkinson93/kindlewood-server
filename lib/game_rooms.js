...
function start(room, userId) {
  ...
  if (engine) {
    room.engine = engine;
    room.state = engine.create(room.seats);
  }
  touch(room);
...
// Host-only: resolve any AI seats the engine is currently waiting on.
// AI lives client-side, so the host's browser polls this with the AI's
// decision. We re-validate that the seat really is AI and really is the
// one the engine expects before applying.
function aiAction(room, userId, msg) {
  if (room.hostId !== userId) return;            // only the host drives AI
  if (!room.engine || !room.state) return;
  const g = room.state;
  const seat = room.seats.find(x => x.seat === msg.seat);
  if (!seat || !seat.isAI) return;
  gameActionBySeat(room, seat.seat, msg);
}
