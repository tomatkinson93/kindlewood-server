...
function pendingAiSeat(room) {
  ...
  const P = g.pending;
  if (room.gameType === 'squirrel') {
    if (g.phase === 'turn') return seatIsAI(g.players[g.turn].seat) ? g.players[g.turn].seat : null;
    if (g.phase === 'squirrel' || g.phase === 'magpie' || g.phase === 'foxdare')
      return seatIsAI(P.actorSeat) ? P.actorSeat : null;
    if (g.phase === 'daredraw')
      return (g.dare && seatIsAI(g.dare.victimSeat)) ? g.dare.victimSeat : null;
    if (g.phase === 'storm') {
...
function finishMatch(room) {
  ...
  try {
    ...
    for (const s of room.seats) {
      ...
      stats.record(s.id, room.gameType, s.seat === winnerSeat).catch(() => {});
    }
