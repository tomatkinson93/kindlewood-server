...
function activeRoomForUser(userId, exceptCode) {
  ...
  for (const r of rooms.values()) {
    ...
    if (r.status === 'lobby') {
      if (r.players.some(p => !_isAI(p) && String(p.id) === uid)) return r;
    } else if (r.status === 'playing') {
      const seats = r.seats || r.players;
      if (seats.some(s => !_isAI(s) && String(s.id) === uid)) return r;
    }
  }
  return null;
}
...
function pushState(room) {
  ...
  broadcast(room, { type: 'game_state' }, (ev, uid) => ({
    type: 'game_state',
    state: { ...room.engine.view(room.state, uid), deadlineAt },
  }));
}
...
const DISCONNECT_GRACE_MS = 60000;    // §3.3: grace before an absent seat becomes AI
...
// pushState, so every state mutation keeps it current. Briar-only.
function _refreshDeadline(room) {
  const g = room.state;
  if (room.gameType !== 'briar' || !g || g.phase === 'gameover') {
    room.deadlineAt = null; room.phaseSig = 'none'; return;
  }
  const sig = _phaseSig(g);
  if (sig !== room.phaseSig) {
    room.phaseSig = sig;
    room.deadlineAt = Date.now() + (BRIAR_PHASE_TIMEOUT[g.phase] || 60000);
  }
}
...
// Enforce the current Briar phase's default when its deadline passes.
...
function _serverTick() {
  ...
  for (const room of rooms.values()) {
    ...
    try {
      // Disconnect grace (§3.3): a human seat absent past the grace window is
      // handed to the AI so the table keeps playing. Done first, so a freshly
      // converted seat is driven this same cycle.
      if (room.absent && room.absent.size) {
        for (const [uid, entry] of room.absent) {
          if (!entry.converted && now - entry.since >= DISCONNECT_GRACE_MS) _convertSeatToAI(room, uid, entry);
        }
      }
      ...
      } else if (room.gameType === 'briar' && room.deadlineAt && now >= room.deadlineAt) {
...
function _convertSeatToAI(room, userId, entry) {
  const seat = room.seats.find(s => s.id === userId);
