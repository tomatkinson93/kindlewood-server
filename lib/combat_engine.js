...
(function (global) {
  ...
  // ── Build a battle ───────────────────────────────────────────────────────
  function createBattle({ players, enemies, scene, seed, deck, deckSeed, formation, enemyFormation }) {
  ...
  function _telegraphCurrent(state) {
    const cur = currentUnit(state);
    if (cur && cur.side === 'enemy' && cur.moves && cur.moves.length) {
      cur._intent = _cards().COMBAT.selectNextMove(state, cur);
      emit(state, { type: 'intent-set', actor_id: cur.id,
        intent: cur._intent ? (cur._intent.intent || 'attack') : 'attack',
        move_name: cur._intent ? cur._intent.name : '' });
    }
  }
