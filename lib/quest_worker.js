...
const clanQuests = require('./clan_quests');

// ── Tunables ──
// 5s default. We tried 30s initially with the reasoning that a low-traffic
// solo game doesn't need fast worker ticks. In practice, the worst-case
// 30s gap between a quest hitting `completes_at` and the worker resolving
// it was very user-visible: the modal sat on "Returning…" for up to half
// a minute. The discovery query is a single index scan returning zero
// rows on most ticks (cost: sub-millisecond). At 5s we do 12 cheap probes
// per minute instead of 2 — the DB doesn't care, and the user-visible
// "Returning…" window drops from 0-30s to 0-5s. If the player count grows
// enough to make probe traffic measurable, raise this back; for one
// settlement on a $7 instance, this is free.
...
async function _tick() {
  ...
  try {
    ...
    // Clan quests: resolve due runs, lapse stale forming parties. Cheap on
    // idle ticks (indexed scans returning nothing); never throws.
    await clanQuests.tick();
  } catch (e) {
    _stats.errors_total++;
    _stats.last_error_at = new Date().toISOString();
