...
function _aiChallengeDecision(g, observer, claimantSeat, role) {
  ...
  // Round 1 strongly damped, easing to full confidence by ~round 3.
  const roundFactor = Math.min(1, 0.45 + 0.28 * (g.round - 1));

  // Role weighting
  const roleW = ROLE_SUSPICION[role] || 1;

  // Desperation: if challenging-or-not, a player on their LAST card who is
  // the target of a lethal action (Sting/Banish) has nothing to lose by
  // challenging — swing hard toward calling it.
  let desperation = 1;
  ...
  if (hiddenLeft === 1 && iAmTarget && lethal) desperation = 2.4;

  // Final probability of calling the challenge
  let chance = bluff * roundFactor * roleW * desperation;

  // Never quite 0 or 1 — keep humans guessing.
...
function aiReact(g, seat) {
  ...
  if (g.phase === 'block') {
    ...
    let bluffBlock = 0.1;
    if (g.pending.targetSeat === p.seat && (g.pending.action === 'sting')) bluffBlock = 0.3;
