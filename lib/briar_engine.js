...
// ── Court Seasons ─────────────────────────────────────────────────────
// Always in play. One symmetric modifier per round. Values are read through the helpers below
...
const SEASON_MODS = {
  harvest:  { forageGain: 2, gatherGain: 3 },
  frost:    { forageGain: 0, gatherGain: 1 },
  festival: { gatherUnblockable: true },
  shadows:  { consultDraw: 3 },
  quiet:    {},
};
const SEASON_LOG = {
  harvest:  '🌕 Harvest Moon rises — foraging yields 2, gathering 3.',
  frost:    '❄️ Frost settles on the Court — foraging yields nothing, gathering only 1.',
...
// Costs are fixed today (no season changes them); kept as the single read
// site so a future modifier needs no call-site changes.
function _actionCost(g, action) { return ACTIONS[action] ? ACTIONS[action].cost : 0; }
// `??` not `||`: Frost's forage yield is a legitimate 0.
function _forageGain(g) { return _mods(g).forageGain ?? 1; }
function _gatherGain(g) { return _mods(g).gatherGain ?? 2; }
...
function _drawSeason(g) {
  ...
  g.seasonSeq = (g.seasonSeq || 0) + 1;   // clients animate on change (a season can repeat across a reshuffle)
...
function create(seats, opts) {
  ...
  const g = {
    ...
    lapAnchor: 0,     // set below: the opener's index — each lap from them is a round
    round: 1,
    actionsThisGame: 0,
    difficulty,
  ...
  // Court Seasons are part of the game. `seasons: false` exists only so
  // tests can isolate other rules from the season modifiers.
  if (!(opts && opts.seasons === false)) {
    g.seasons = true;
    g.season = null;
...
function nextTurn(g) {
  ...
  // A round is one full lap starting from whoever opened the game (the lap
  // anchor). Stepping onto the anchor's chair — even if they're out and the
  // marker skips past — starts a new round and turns a new season. (It used
  // to key off seat 0, so a game opened by the last seat got a new season
  // after a single turn.)
  let newRound = false;
  do {
    g.turn = (g.turn + 1) % g.players.length;
    if (g.turn === g.lapAnchor) newRound = true;
  } while (!g.players[g.turn].alive);
  if (g.seasons && newRound) _drawSeason(g);
...
function resolve(g) {
  ...
  switch (action) {
    ...
    case 'gather': actor.acorns += _gatherGain(g); break;
...
function _scoreAction(g, p, cand, persona) {
  ...
  switch (cand.action) {
    case 'forage': v = _forageGain(g); break;                                    // seasons shift both yields
    case 'gather': v = _gatherGain(g) * (0.9 + 0.2 * persona.greed); break;      // greed leans gather over forage
