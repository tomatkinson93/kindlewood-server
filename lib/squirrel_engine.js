...
function _bankActive(g, p) {
  ...
  log(g, `${p.name} banks their stash from last turn (${p.stash.length} card${p.stash.length === 1 ? '' : 's'}).`);
  p.banked.push(...p.stash);
  p.stash = [];
}
...
function endGame(g) {
  ...
  for (const p of g.players) { if (p.stash.length) { p.banked.push(...p.stash); p.stash = []; } }
  let best = null;
