...
// Ledger-friendly phrasing for a value card ("7 acorns", "the Golden Acorn").
// Specials narrate themselves in draw(), so this only covers bankable cards.
function drawPhrase(c) {
  if (c.kind === 'number') return `${c.num} acorn${c.num === 1 ? '' : 's'}`;
  if (c.kind === 'lucky7') return 'a Lucky 7';
  if (c.kind === 'rotten') return 'a Rotten Acorn (−7)';
  if (c.kind === 'golden') return 'the Golden Acorn (20)';
  return label(c);
}
const _SPECIAL_KINDS = ['squirrel', 'storm', 'burrow', 'magpie', 'foxdare', 'pact', 'badger'];
...
function create(seats, opts) {
  ...
  const g = {
    deck, players,
    // The turn order is FIXED at deal: the players array in seat order. g.turn
    // is a pointer into it that only ever advances by one (endTurn), wrapping at
    // the end. A Fox's Dare is a temporary excursion that never moves this
    // pointer off the caller (see endTurn's guard). `order` exposes the sequence
    // to the client for the turn-order strip.
    order: players.map(p => p.seat),
    ...
    pending: null,
    log: [],
    ledger: [],   // full, uncapped-ish history for the ledger modal (§ turn order)
    winner: null,
    difficulty,
  };
  log(g, "The squirrel's hoard is tipped onto the table. Draw, but mind the duplicates!");
  led(g, `${cur(g).name}'s turn.`, 'turn');
...
function log(g, m) { g.log.push(m); if (g.log.length > 16) g.log.shift(); ledPush(g, m, 'event'); }
// The ledger keeps the full narrated history (the on-table log is a short
// sliding window). `led` adds a ledger-only line with a semantic kind for
// styling; ledPush is the internal appender log() also feeds.
function led(g, m, kind) { ledPush(g, m, kind || 'event'); }
function ledPush(g, m, kind) {
  if (!g.ledger) g.ledger = [];
  g._ledSeq = (g._ledSeq || 0) + 1;
  g.ledger.push({ seq: g._ledSeq, text: m, kind: kind || 'event' });
  if (g.ledger.length > 400) g.ledger.shift();
}
...
function draw(g, seat) {
  ...
  // Value cards get a single ledger-style draw line (with a risk marker once
  // past the three safe draws); specials narrate themselves below.
  if (!_SPECIAL_KINDS.includes(card.kind)) {
    const risky = p.drawsThisTurn > 3 ? ' (no longer safe!)' : '';
    log(g, `${p.name} drew ${drawPhrase(card)}${risky}.`);
  }
...
function bank(g, seat, force) {
  if (!force) {
    if (g.phase !== 'turn' || cur(g).seat !== seat) return false;
    if (g.dare && seat === g.dare.victimSeat) return false; // a dare victim must draw, not bank out
...
function endTurn(g) {
  // TURN-ORDER GUARD: a dare is a temporary excursion — the victim draws in the
  // caller's place, but the real round-robin must never advance off the victim's
  // seat. Any path that "ends a turn" while a dare is active (a resonating Forest
  // Pact, an errant bank, or any future effect) is rerouted through _endDare so
  // control always snaps back to the caller. Bust/burrow already call _endDare
  // directly and clear g.dare before reaching here, so this only catches the
  // stray paths. This is the single chokepoint that keeps turn order intact.
  if (g.dare && cur(g).seat === g.dare.victimSeat) {
  ...
  g.phase = 'turn';
  g.pending = null;
  led(g, `${cur(g).name}'s turn.`, 'turn');   // ledger turn header (§ turn order)
}
...
function resolveFoxDare(g, seat, targetSeat) {
  ...
  log(g, `${bySeat(g,seat).name} played the Fox on ${t.name} — ${t.name} must draw ${remaining}!`);
...
function view(g, forId) {
  return {
    ...
    phase: g.phase,
    turn: g.turn,
    // The seat currently acting (dare victim if a dare is running, else the
    // normal turn holder) and the fixed turn order for the client strip.
    turnSeat: g.dare ? g.dare.victimSeat : (g.players[g.turn] ? g.players[g.turn].seat : null),
    order: g.order || g.players.map(p => p.seat),
    // The seat whose turn it REALLY is — during a dare that's the caller, held
    // while the victim draws in their place; otherwise the same as turnSeat.
    homeSeat: g.dare ? g.dare.returnSeat : (g.players[g.turn] ? g.players[g.turn].seat : null),
    ...
    dare: g.dare ? { victimSeat: g.dare.victimSeat, returnSeat: g.dare.returnSeat, remaining: g.dare.remaining } : null,
    ledger: (g.ledger || []).slice(-200),
