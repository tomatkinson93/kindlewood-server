...
function draw(g, seat) {
  ...
  switch (card.kind) {
    ...
    case 'burrow':
      ...
      if (g.dare && p.seat === g.dare.victimSeat) {
        // Burrow overrides the dare: bank the victim, drop the dare, then end
        // the actor's real turn.
        if (p.stash.length) { const n = _bankStash(p); log(g, `${p.name} banks ${n}.`); }
        const d = g.dare; g.dare = null;
        p.drawsThisTurn = d.savedDraws;
        g.turn = d.savedTurn;
        endTurn(g);
        return true;
      }
  ...
  if (!safe && myCopies >= 2 && !stoleFrom) {
    ...
    if (p.badgerArmed) {
      _consumeOneBadger(p);
      log(g, `${p.name} would bust — but a Badger absorbs it!`);
      return _afterDraw(g, p);
    }
    return _bust(g, p, card);
  }
  ...
  if (!safe && myCopies >= 2 && stoleFrom) {
    if (p.badgerArmed) { _consumeOneBadger(p); log(g, `${p.name}'s Badger absorbs the clash!`); return _afterDraw(g, p); }
    return _bust(g, p, card);
  }
...
function _bust(g, p, card) {
  ...
  p.busted = true;
  p.badgerArmed = false;
  // A bust during a dare ends the dare and returns to the actor's real turn.
  if (g.dare && p.seat === g.dare.victimSeat) {
    const d = g.dare; g.dare = null;
    p.busted = false;
    p.drawsThisTurn = d.savedDraws;
...
function _resolveKeptCard(g, p, card) {
  ...
  if (p.drawsThisTurn > 3 && copies >= 2) {
    if (p.badgerArmed) { _consumeOneBadger(p); log(g, `${p.name}'s Badger absorbs it!`); return true; }
    return _bust(g, p, card);
  }
  return true;
}
...
function _dareAdvance(g) {
  ...
  // If the victim busted during the dare, the dare ends.
  if (victim.busted) { return _endDare(g, true); }
  ...
  return _endDare(g, false);
}

function _endDare(g, busted) {
  const d = g.dare;
  g.dare = null;
  if (!d) { g.phase = 'turn'; g.pending = null; return true; }
  const victim = bySeat(g, d.victimSeat);
  victim.busted = false;
  // Restore the dared player's pre-dare draw count (their own turn, if it was
  // theirs, continues; if it was someone else's dare, they were just a victim).
  victim.drawsThisTurn = d.savedDraws;
  // Restore the original turn to the actor who played Fox's Dare.
  g.turn = d.savedTurn;
  g.phase = 'turn';
  g.pending = null;
  return true;
...
function _forcedResolveCard(g, t, card) {
  if (card.kind === 'lucky7') {
    if (t.stash.some(c => c.kind === 'lucky7')) {
      if (t.badgerArmed) { _consumeOneBadger(t); log(g, `${t.name}'s Badger absorbs a second Lucky 7!`); return; }
      log(g, `${t.name} draws a second Lucky 7 and BUSTS!`); t.stash = []; t.busted = true; return;
    }
    t.stash.push(card); return;
  }
  ...
  if (card.kind === 'burrow') {
    const n = _bankStash(t);
    if (n) log(g, `${t.name}'s forced Burrow banks ${n}.`);
    return;
  }
  if (card.kind === 'pact') {
    const other = g.players.find(o => o !== t && o.stash.some(c => c.kind === 'pact'));
    if (other) {
      log(g, `Forest Pacts resonate — ${t.name} and ${other.name} both bank!`);
      for (const who of [t, other]) _bankStash(who);
    } else { t.stash.push(card); log(g, `${t.name} lays down a Forest Pact.`); }
    return;
  }
  if (card.kind === 'magpie') {
...
function _forcedDrawResolve(g, t, card) {
  ...
  if (copies >= 2) {
    if (t.badgerArmed) { _consumeOneBadger(t); log(g, `${t.name}'s Badger absorbs the dare's clash!`); return; }
