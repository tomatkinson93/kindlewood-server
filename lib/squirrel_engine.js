...
function draw(g, seat) {
  ...
  // Special effect cards trigger immediately
  switch (card.kind) {
    case 'squirrel': return _doSquirrel(g, p, card);
    case 'storm':    p.stash.push(card); return _doStorm(g, p);
    case 'burrow':   p.stash.push(card); log(g, `${p.name} burrows — banking everything.`); return bank(g, seat, true);
    case 'magpie':   p.stash.push(card); return _enterMagpie(g, p);
    case 'foxdare':  p.stash.push(card); return _enterFoxDare(g, p);
    case 'pact':     p.stash.push(card); return _doPact(g, p);
  ...
  if (stoleFrom) log(g, `${p.name} steals ${stoleFrom.count} × ${label(card)} from ${stoleFrom.name}!`);
...
function bank(g, seat, force) {
  ...
  if (p.stash.length) {
    log(g, `${p.name} banks ${p.stash.length} card${p.stash.length === 1 ? '' : 's'}.`);
    p.banked.push(...p.stash);
    p.stash = [];
  }
  endTurn(g);
  return true;
...
function endTurn(g) {
  const p = cur(g);
  // Auto-bank surviving Lucky 7s (only if not busted this turn)
  if (!p.busted && p.stash.some(c => c.kind === 'lucky7')) {
...
function _resolveKeptCard(g, p, card) {
  // Re-run the number/special logic for the kept card without a fresh draw count
  if (['magpie','burrow','pact','badger','storm','squirrel','foxdare','lucky7'].includes(card.kind)) {
    // treat as if drawn (these are rare nested cases) — push and trigger
    if (card.kind === 'lucky7') { p.stash.push(card); return true; }
    if (card.kind === 'burrow') { p.stash.push(card); return bank(g, p.seat, true); }
    if (card.kind === 'badger') { p.stash.push(card); p.badgerArmed = true; return true; }
    if (card.kind === 'storm') { p.stash.push(card); return _doStorm(g, p); }
    if (card.kind === 'magpie') { p.stash.push(card); return _enterMagpie(g, p); }
    if (card.kind === 'foxdare') { p.stash.push(card); return _enterFoxDare(g, p); }
    if (card.kind === 'pact') { p.stash.push(card); return _doPact(g, p); }
...
function resolveStorm(g, seat, cardIndex) {
  ...
  if (p.stash.length) {
    ...
    const [card] = p.stash.splice(idx, 1);
    g.deck.push(card); shuffle(g.deck);
    log(g, `${p.name} returns ${label(card)} to the hoard.`);
  }
...
function resolveFoxDare(g, seat, targetSeat) {
  ...
  g.phase = 'turn'; g.pending = null;
  // The victim draws 3 immediately, resolving normally, into THEIR stash.
  // We temporarily resolve as that player without changing whose turn it is.
  for (let i = 0; i < 3; i++) {
...
function _forcedDraw(g, t) {
  ...
  // Specials on a forced draw resolve simply (no nested prompts for the dare).
  if (card.kind === 'lucky7') { t.stash.push(card); return; }
  if (['magpie','burrow','pact','badger','squirrel','storm','foxdare'].includes(card.kind)) {
    // For simplicity, forced special draws just go to stash (no chain effects).
    t.stash.push(card);
    if (card.kind === 'badger') t.badgerArmed = true;
    return;
  }
  const key = matchKey(card);
...
function _doPact(g, p) {
  ...
  if (other) {
    ...
    for (const who of [p, other]) {
      if (who.stash.length) { who.banked.push(...who.stash); who.stash = []; }
    }
    // p's turn ends after banking
    endTurn(g);
    return true;
  }
  log(g, `${p.name} holds a Forest Pact, waiting for its match.`);
  return _afterDraw(g, p);
}
...
module.exports = {
  create, draw, bank,
  resolveSquirrel, resolveStorm, resolveMagpie, resolveFoxDare,
  stormPending, view, aiResolve, score,
};
if (typeof window !== 'undefined') window.SquirrelEngine = module.exports;
