...
function draw(g, seat) {
  ...
  switch (card.kind) {
    ...
    case 'badger': {
      ...
      const bc = p.stash.filter(c => c.kind === 'badger').length;
      log(g, bc > 1 ? `${p.name} adds another Badger — ${bc} now stand guard.` : `${p.name} tucks a Badger away — the next bust will be ignored.`);
