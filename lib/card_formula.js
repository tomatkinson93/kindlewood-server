...
(function (root) {
  ...
  function parseFormula(formula) {
    ...
    lines.forEach(function (raw, idx) {
      ...
      // Optional positional condition prefix: "if_front <verb>: expr" / "if_back".
      var cond = null;
      var m = line.match(/^(if_front|if_back)\s+/i);
      if (m) { cond = m[1].toLowerCase(); line = line.slice(m[0].length); }
      ...
      ops.push({ verb: verb, param: param, eval: ev, cond: cond });
    });
  ...
  function compileEffect(formula) {
    ...
    return function (state, actor, target, ctx) {
      ...
      for (var i = 0; i < ops.length; i++) {
        var op = ops[i];
        // Positional condition: skip this op unless the actor is front/back.
        if (op.cond === 'if_front' && ctx.isFront && !ctx.isFront(actor)) continue;
        if (op.cond === 'if_back' && ctx.isBack && !ctx.isBack(actor)) continue;
